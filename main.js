...
let phara =document.getElementById('p')
...
let inputtag = document.getElementById('amount').value
    const value = parseInt(inputtag);
    const random = Math.floor(Math.random() * arr.length)

    console.log(value)
    if(value===1 || value > 10){
        phara.innerHTML =`<p>${arr[random]}</p></br>` 

    }
    else{
        let newp = arr.slice(0,value).map((e)=>{
                return `<p>${e}</p></br>`
        }).join('')
        phara.innerHTML = newp
        console.log(newp)
    }
